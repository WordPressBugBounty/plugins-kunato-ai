	...
	*/
	public function kunato_options_settings_init(){
		...
		add_settings_section(
			...
			__('Kunato Identifier.', 'kunato-ai'),
		...
		// add_settings_field(
		// 	'kunato_disable_title_modify',
		// 	__('Disable title modify', 'kunato-ai'),
	...
	?>
		<p id="<?php echo esc_attr($args['id']); ?>"><?php esc_html_e('Please enter an identifier.', 'kunato-ai'); ?></p>
	<?php
	}
	...
	public function kunato_options_identification_name_cb($args)
	{
		...
		<input
			...
			value="<?php echo esc_attr( get_option('kunato_identification') ); ?>"
			placeholder="<?php esc_attr_e( 'Enter Identifier', 'kunato-ai' ); ?>"
		/>
		<?php
	}
	...
	?>
		<select id="<?php echo esc_attr($args['label_for']); ?>" name="<?php echo esc_attr($args['label_for']); ?>">
			<option value=""><?php esc_html_e('Select Currency', 'kunato-ai'); ?></option>
			<option value="inr" <?php selected($options, 'inr'); ?>><?php esc_html_e('INR', 'kunato-ai'); ?></option>
			<option value="usd" <?php selected($options, 'usd'); ?>><?php esc_html_e('USD', 'kunato-ai'); ?></option>
		</select>
	<?php
	}
	...
	public function kunato_options_page_html()
	{
		...
		// if WP has posted back, verify our nonce
		if ( isset($_SERVER['REQUEST_METHOD']) === 'POST' ) {
			// this will die() with “Cheatin' uh?” if the nonce is invalid
			check_admin_referer( 'kunato_save_settings', 'kunato_nonce' );
		}
		...
		if (isset($_GET['settings-updated'])) {
			...
			add_settings_error('kunato_messages', 'kunato_message', __('Settings Saved', 'kunato-ai'), 'updated');
		}
	...
	?>
		<div class="wrap">
			...
			<form action="options.php" method="post">
				<?php
				// add our custom nonce field
				wp_nonce_field( 'kunato_save_settings', 'kunato_nonce' );
				...
				submit_button(__('Save Settings', 'kunato-ai'));
				?>
			</form>
		</div>
	<?php
